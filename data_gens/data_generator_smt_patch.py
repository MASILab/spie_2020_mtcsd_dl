...
def nifti_smt_generator_patch(inputPath, bs, patch_size):
    # open the CSV file for reading
    #f = open(inputPath, "r")
    n_retrievals = 50000
    n_classes = 3
    ...
    while True:
        ...
        for vol_index, each_vol in enumerate(inputPath):
            ...
            #output_vol = load_nifty(each_vol['output_image'], data_type='float32')
            #frac_vol = load_nifty(each_vol['vol_frac'], data_type='float32')
            ...
            while current_retrieval < n_retrievals:
                ...
                for each_ind, each in enumerate(rand_inds):
                    ...
                    # Just the first FOD SH Coefficient
                    #images[each_ind, :, :, :, 0] = np.squeeze(input_vol[vox_inds[0]-1:vox_inds[0]+2,
                    #                                                    vox_inds[1]-1:vox_inds[1]+2,
                    #                                                    vox_inds[2]-1:vox_inds[2]+2, 0]
                    #                                          )

                    # Tissue Fraction estimates
                    images[each_ind, :, :, :, :] = np.squeeze(input_vol[vox_inds[0] - 1:vox_inds[0] + 2,
                                                                        vox_inds[1] - 1:vox_inds[1] + 2,
                                                                        vox_inds[2] - 1:vox_inds[2] + 2, 45:48]
                                                                        )
